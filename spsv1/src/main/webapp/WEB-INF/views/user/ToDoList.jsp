			...
			<div class="spal">
				<div class="panel-group">
				    ...
				    <c:if test="${user.role =='TL' }">
				      ...
				      <form action="/spsv1/task/allocateTask" method="post" commandName="member">
				      <div class="panel-body" id="rstask">
				      	   <input type="hidden" name="amtrange" value="${amtrange}">
				      	<div class="row rtask">
				...
				<tr>
								...
								<%-- <td><input type="checkbox" name="checkassigntask" value="${list.getPolicyno() }"></td> --%>
								 <%-- <td>${list.getSubtypeteam()}</td> --%>
								  <td>${list.getPaymentMode()}</td>
				      		  <!--  <td></td> -->
				      		   <td> 
				      		   Call ID: ${list.getCall_id()}<br>
				      		   
				      		   <c:choose>
									<c:when test="${list.getFinalRemarkOners() !=null }">
										Policy No:  <a href="/spsv1/cheque/displayPolicyDetails?policyNo=${list.getPolicyno() }&prnno=${list.getPrnNo() }&paymentMode=${list.getPaymentMode()}&onersremark=${list.getFinalRemarkOners()}">${list.getPolicyno() }</a><br>
									</c:when>
									<c:otherwise>
										Policy No:  <a href="/spsv1/cheque/displayPolicyDetails?policyNo=${list.getPolicyno() }&prnno=${list.getPrnNo() }&paymentMode=${list.getPaymentMode()}&onersremark=null">${list.getPolicyno() }</a><br>
									</c:otherwise>
								</c:choose>
				      			...
				      			PRN No: ${list.getPrnNo() } <br> 
				      		Customer Name: ${list.getName() } <br>
				      		DESCR: ${list.getDescr() } <br>

...
<center>
	<div class="report">
	<h2>Search Sampling Policy Details</h2>
		<form action="/spsv1/report/searchuserdata" method="post">
		...
		<%-- <form action="" method="">
			Policy No. <input type="text" name="policyno" id="policyno" />
			<input class="btn btn-success" id="searchpolicy" type="button" value="Submit">
		</form> --%>
		...
		<table class="table table-responsive table bordered report_tbl">
			...
			<c:forEach var="list" items="${searchulist }">
				<tr>
					<%-- <td>${list.policyno }</td> --%>
					<td><a href="/spsv1/report/editpolicy?policyNo=${list.getPolicyno() }&prnno=${list.getPrnNo() }">${list.getPolicyno() }</a></td>
					<td>${list.remarksBy }</td>
					<td>${list.remarks }</td>
					<td>${list.date }</td>
				</tr>
			</c:forEach> 

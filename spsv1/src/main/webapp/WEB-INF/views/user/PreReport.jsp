...
<center>
	<div class="report">
		<form action="/spsv1/Prereport/show" method="post">
		...
		<table class="table table-responsive table bordered report_tbl pre_reportTable">
			<tr>
				<th>Name</th>
			
			    <th class="maker">Maker Allocated Task</th>
				<th class="maker">Maker Pending Task</th>
				<th class="maker">Maker Complete Task</th>
				
				
				<th class="checker">Checker Allocated Task</th>
				<th class="checker">Checker Pending Task</th>
				<th class="checker">Checker Complete Task</th>
			
			
				<th class="total">Total Allocated Task</th>
				<th class="total">Total Pending Task</th>
				<th class="total">Total Complete Task</th>
			</tr>
			
			<c:forEach var="list" items="${report }">
			<%-- <c:if test="${list.mkraloctsk > 0 ||  list.chraloctsk > 0 || list.allocate > 0 || list.mkrpendtsk > 0 || list.chrpentsk > 0 || list.pending > 0 || list.mkrcomplt>0 || list.chrcmplt > 0 || list.complete > 0}"> --%>
			<tr>
				<th>${list.username}</th>
				
				<td class="maker">${list.mkraloctsk }</td>
				<td class="maker">${list.mkrpendtsk }</td>
				<td class="maker">${list.mkrcomplt }</td>
				
				<td class="checker">${list.chraloctsk }</td>
				<td class="checker">${list.chrpentsk }</td>
				<td class="checker">${list.chrcmplt }</td>
				
				<td class="total">${list.allocate }</td>
				<td class="total">${list.pending }</td>
				<td class="total">${list.complete }</td>
				</tr>
				<%-- </c:if> --%>
			</c:forEach>

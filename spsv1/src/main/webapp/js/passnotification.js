$(document).ready(function(){
	...
	var strDate = d.getFullYear() + "-" + twoDigitMonth + "-" + twoDigitDate; 
	
	/*var d = new Date();
	var twoDigitDate = ((d.getDate().length+1) === 1)? (d.getDate()+1) : '0' + (d.getDate());
	var twoDigitMonth = ((d.getMonth().length+1) === 1)? (d.getMonth()+1) : '0' + (d.getMonth()+1);
	if(twoDigitDate.length == 3) {
	...
	$("#newpassword").on('keypress', function(e){
		...
		//alert($("#newpassword").val().length);
		
		if($("#newpassword").val().length == 15) {
			$.confirm({
	    	    title: 'Error!',
	    	    content: "Password length exceed.",
	    	    buttons: {
	    	        OK: function () 
	    	        {
	    	        	return true;
	...
	$("#confirmpassword").on('keypress', function(e){
		...
		if($("#confirmpassword").val().length == 15) {
			$.confirm({
	    	    title: 'Error!',
	    	    content: "Password length exceed.",
	    	    buttons: {
	...
	$(".editdata").click(function(){
		...
		var passwordvalid = /^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[~`!@#$%^&*()-_+=])[a-zA-Z0-9~`!@#$%^&*()-_+=]+$/;
		...
		if(oldpassword=="" ||newpassword=="" || confirmpassword=="")
		{
    		...
    		$.ajax({
                ...
                success:function(data)
                {
                	...
                	else if((newpassword.length < 8 || newpassword.length > 15) || (confirmpassword.length < 8 || confirmpassword.length > 15))
	        			{
	                		$.confirm({
	            	    	    title: 'Error',
	            	    	    content: "Please Enter minimum 8 and maximum 15 characters Password.....!!!!!",
	            	    	    buttons: {
	            	    	        OK: function () {
	            	    	        	document.getElementById("newpassword").value = "";
	            	    	        	document.getElementById("confirmpassword").value = "";
	            		...
	            		else if (!newpassword.match(passwordvalid))
		       		    {
	                		$.confirm({
	            	    	    title: 'Error',
	            	    	    content: "Password must contain at least 1 capital letter, 1 small letter, 1 number and 1 special character......!!!!!",
	            	    	    buttons: {
	            	    	        OK: function () {
	            	    	        	document.getElementById("newpassword").value = "";
	            	    	        	document.getElementById("confirmpassword").value = "";
